...
for(age_type in c("ado", "adult")){
  ...
  if(age_type == "ado"){
    ...
    subset_level <- data_level %>% filter(variable %in% c("prev_bmi_2sd", "prev_bmi_neg2sd", "prev_double_burden_ado"))
    subset_level$variable <- factor(subset_level$variable, levels = c("prev_bmi_neg2sd","prev_bmi_2sd","prev_double_burden_ado"))
    levels(subset_level$variable) <- c('Thinness', 'Obesity', 'Combined burden')
    data_sources <- data_sources_ado
  }else{
    ylims <- c(-3,90)
    breaks <- c(0,20,40,60,80)
    # fill_scale <- fill_scale_double_burden
    subset_level <- data_level %>% filter(variable %in% c("prev_bmi_l185", "prev_bmi_30", "prev_double_burden"))
    subset_level$variable <- factor(subset_level$variable, levels = c("prev_bmi_l185","prev_bmi_30","prev_double_burden"))
    levels(subset_level$variable) <- c('Underweight', 'Obesity', 'Combined burden')
