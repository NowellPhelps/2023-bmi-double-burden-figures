...
for(age_type in c("ado", "adult")){
  ...
  for (my_country in unique(countrylist$Country)){
    for(my_sex in sexes){
      ...
      p1 <- ggplot(d_plot %>% filter(!(variable == 'Proportion of\ncombined burden\ncomposed of obesity')),
                   aes(x = year, y = mean, colour = Superregion)) +
      ...
      p2 <- ggplot(d_plot %>% filter(variable == 'Proportion of\ncombined burden\ncomposed of obesity'),
                   aes(x = year, y = mean, colour = Superregion)) +
        geom_hline(yintercept = 0, linetype = 'dashed', colour = grey(0.5)) +
        geom_hline(yintercept = 50, linetype = 'dashed', colour = grey(0.7)) +
        geom_hline(yintercept = 100, linetype = 'dashed', colour = grey(0.5)) +
        geom_errorbar(aes(ymin = l, ymax = u), width = 4) +
        geom_point(size = 2.5) +
        theme_bw() +
        theme(axis.title.x = element_blank(),
              ...
              panel.grid.minor.y = element_line(colour = grey(0.95))) +
        scale_x_continuous(expand=c(0,0), limits = c(1970,2042), breaks = c(1990, 2022)) +
        scale_y_continuous(expand=c(0,0), limits = c(-3,103), breaks = breaks) +
      ...
      ps[[paste(age_type, my_country, my_sex)]] <- arrangeGrob(p1  + theme(legend.position = "none"),
                                                               blank,
                                                               arrangeGrob(
                                                                 blank,
                                                                 ...
                                                                 ncol = 1, 
                                                                 heights = c(1,20)),
                                                               nrow = 1, widths = c(3,0.1, 1.15))

    }
  }

  ps[[paste(age_type, "legend")]] <- get_legend(p1)
}



########################## PRINT PDF ###########################################
cairo_pdf(paste0(outdir_folder, ifelse(appendix, "Appendix Figure ", "Figure "), figNum, figsuffix,".pdf"), height = 12, width = 15.5, onefile=T)
    ...
    arrangeGrob(
      ...
      arrangeGrob(
        ...
        arrangeGrob(
          ...
          ncol = 1, heights = c(.2,.5,.1,5,0.1,.5,.1,5,0.3)
        ),
        blank,

        nrow = 1,
        widths = c(.05,1,.2,1,0.05)
      ),
      blank,
      blank,
      nrow = 4,
