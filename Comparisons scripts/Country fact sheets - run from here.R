   ...
   text_sources[[paste(my_country, age_type)]] <- get_text_sources(my_country, age_type, data_sources)
    ...
    for(my_sex in sexes){
      ...
      for(my_variable in vars_of_interest){
         ...
         ps[[paste("region rank plot", my_variable, my_country, my_sex, age_type)]] <- make_region_rank_one_year(data_level, data_ranking, my_country, my_sex, my_variable, my_year = plot.end.year, age_type, returnLeg = F, rank_plot = F)
