...
make_victor_scatter <- function(plot_data, my_country, my_sex, my_x_variable, my_y_variable, my_year, age_type, plotLeg = F, option = "region"){
   #  option takes "country" or "region"
   # "country" highlights country only
   # "region" highlights country and other countries in region
   ...
   plot_data <- plot_data %>% 
      filter(year == my_year, variable %in% c(my_x_variable, my_y_variable)) %>%
      mutate(col_ind = ifelse(Country == my_country, "country", "non")) %>%
      select(-c(l, u, se)) %>%
      mutate(variable = ifelse(variable == my_x_variable, "xvar", "yvar")) %>%
      pivot_wider(names_from = variable, values_from = mean)
   
   ylims <- c(0, ceiling(max(plot_data$yvar)))
   xlims <- c(0, ceiling(max(plot_data$xvar)))
   
   plot_data <- plot_data %>% 
      filter(sex == my_sex)
   
   plot.title.y <- switch(my_y_variable, 
                        "prev_bmi_neg2sd" = "Prevalence thinness (%)",
                        "prev_bmi_2sd" = "Prevalence obesity (%)",
                        "prev_bmi_l185" = "Prevalence underweight (%)",
                        "prev_bmi_30" = "Prevalence obesity (%)")
   
   plot.title.x <- switch(my_x_variable, 
                          "prev_bmi_neg2sd" = "Prevalence thinness (%)",
                          "prev_bmi_2sd" = "Prevalence obesity (%)",
                          "prev_bmi_l185" = "Prevalence underweight (%)",
                          "prev_bmi_30" = "Prevalence obesity (%)")
   ...
   p <- ggplot(plot_data, aes(x = xvar, y = yvar, colour = col_ind, alpha = 0.8)) +
      geom_point(size = 1) +
      scale_colour_manual(values = c("country" = "blue", "region" = "grey30", "non" = "grey70"),
                        labels = c("country" = "halp", "region" = my_region, "non" = "Other"),
                        limits = c("region", "non")) +
      new_scale_fill() +
      geom_point(plot_data %>% filter(Country == my_country),
...
get_source_text<- function(N_all, N_natl, my_country, my_sex, age_type){
   ...
   text <- paste0("• Number of studies: ", N_all, " (", N_natl, " nationally representative)*")
...
get_text_description <- function(data_level, data_change, my_country, my_sex, age_type, n_sources_all, n_sources_natl){
   ...
   # Commented out have uncertainty
   #text1 <- paste0('• ', underweight_name, ': ', round(prev_underweight),'% (',round(l_underweight),'-',round(u_underweight),') in 2022, ',get_change_text(underweight.pp.increase, underweight.change, l_underweight.change, u_underweight.change),' from 1990. ', my_country, ' was ranked ', rank_region_underweight, ' in ', my_region, ' and ', rank_world_underweight, ' in the world.')
   #text2 <- paste0('• Obesity: ', round(prev_obesity),' % (',round(l_obesity),'-',round(u_obesity),') in 2022, ',get_change_text(obesity.pp.increase, obesity.change,l_obesity.change, u_obesity.change), ' from 1990. ', my_country, ' was ranked ', rank_region_obesity, ' in ', my_region, ' and ', rank_world_obesity, ' in the world.')
   
   text1 <- paste0('• ', underweight_name, ': ', round(prev_underweight),'% in 2022, ',get_change_text(underweight.pp.increase, underweight.change, l_underweight.change, u_underweight.change, uncertainty = F),' from 1990. ', my_country, ' was ranked ', ordinal_suffix(rank_region_underweight), ' out of ', max(ranking_data$ranking_region),' countries in ', my_region, 'and ', ordinal_suffix(rank_world_underweight), ' out of 200 countries in the world.')
   text2 <- paste0('• Obesity: ', round(prev_obesity),'% in 2022, ', get_change_text(obesity.pp.increase, obesity.change,l_obesity.change, u_obesity.change, uncertainty = F), ' from 1990. ', my_country, ' was ranked ', ordinal_suffix(rank_region_obesity), ' in ', my_region, ' and ', ordinal_suffix(rank_world_obesity), ' in the world.')
