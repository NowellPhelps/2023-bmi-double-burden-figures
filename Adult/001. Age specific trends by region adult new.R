...
ages <- c(19, seq(22.5,82.5,by=5), 88.9)
names(ages) <- unique(res$age_group)
res$age <- ages[res$age_group]

ps <- list()
for(sregion in sregion_order){
  for (plot.year in c(plot.start.year, plot.end.year)){
    p <- ggplot(res %>% filter(Superregion == sregion, year == plot.year), aes(x = age, y = mean)) +
...
plot.title <- paste0("Age trends in ", get_var_longname(variable)," for ", ifelse(sex == "male","men", "women"))
