...
if(age_type == "adult"){
  if(variable_type == "prev"){
    variables    <- c("prev_double_burden", "prev_bmi_30", "prev_bmi_l185", "prev_bmi_30_proportion_double_burden")
  ...
  if(variable_type == "prev"){
    variables    <- c("prev_double_burden", "prev_bmi_2sd", "prev_bmi_neg2sd", "prev_bmi_2sd_proportion_double_burden")
    change_type = "relative"
  } else{
    variables    <- c("mean_bmi")
    change_type = "absolute"
...
# 
# for (my_variable in variables){
#   cairo_pdf(paste0(outdir_folder, "Maps of level and change in ", get_var_longname(my_variable), ".pdf"), height = 35, width = 35, onefile=T)
#   
...
data_velocity     <- read_data_slopes(variables, sexes, age_type = "ageStd",region_level = "Country", slope_type = "velocity") %>%
...
for(my_sex in sexes){
  for (my_variable in variables){
    ...
    ps[[paste("Map velocity", my_variable, my_sex, my_age_group, plot.end.year - 0.5)]]   <- maps_velocity_level_plots(data = subset_velocity, variable = my_variable, my_sex, plot.year = plot.end.year - 0.5, figNum = 5)
    ps[[paste("Map velocity", my_variable, my_sex, my_age_group, plot.start.year + 0.5)]] <- maps_velocity_level_plots(data = subset_velocity, variable = my_variable, my_sex, plot.year = plot.start.year + 0.5, figNum = 5)
...
blank <- grid.rect(gp=gpar(col=NA, fill = NA))
my_age_group <- "ageStd"
outdir_folder <- paste0(maindir,"Postprocessing/Figures/Velocity plots/")

for (my_variable in variables){
  
  cairo_pdf(paste0(outdir_folder, "Maps of velocity and change in velocity of ", my_variable, ".pdf"), height = 21, width = 21, onefile=T)
  
  plot.title <- paste0("Maps of velocity and change in age standardised ", get_var_longname(my_variable))
  
  grid.arrange(
    arrangeGrob(
      textGrob(plot.title,hjust=0,just = c("left"),x = unit(0.01, "npc"),gp = gpar(col = "black", fontsize = 20)),
      
      arrangeGrob(arrangeGrob(textGrob("Female",hjust=0,just = c("left"),x = unit(0.01, "npc"),gp = gpar(col = "black", fontsize = 15)),
                              ...
                              nrow = 6, heights = c(0.5,10, 0.5, 10, 0.5, 10)),
                  blank,
                  arrangeGrob(textGrob("Male",hjust=0,just = c("left"),x = unit(0.01, "npc"),gp = gpar(col = "black", fontsize = 15)),
