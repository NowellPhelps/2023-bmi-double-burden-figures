...
######################### LOOKING AT COUNTRIES PREVALENCE OF SEVERE OBESITY 2021 v PREV OBESITY 1990 ##############
...
for (my_sex in sexes){
  for(my_variable in c("prev_bmi_30", "prev_bmi_l185")){
    ...
    ps[[paste(my_variable, my_sex, "change v level")]] <- ggscatter(data_timechange %>% filter(sex == my_sex & variable == my_variable), 
      ...
      xlab(paste0("Level of ", ifelse(my_variable == "prev_bmi_l185", "underweight", "obesity"), " in ", plot.start.year," (%)")) +
      ylab(paste0("Change ", ifelse(my_variable == "prev_bmi_l185", "underweight", "obesity"), " ", plot.start.year, "-", plot.end.year, " (percentage points)"))  
  }
  ...
  ps[[paste("prev_bmi_30", my_sex, "change v level", "test 5%")]] <- ggscatter(data_timechange %>% filter(sex == my_sex & variable == "prev_bmi_30" & start_level >=5), 
                                                                               ...
                                                                               add.params = list(color = "blue", fill = "lightgray"),
                                                                               conf.int = TRUE) +
    xlab(paste0("Level obesity ", plot.start.year, " (%)")) +
    ylab(paste0("Absolute change in obesity ", plot.start.year, "-", plot.end.year," (percentage points)")) +
    stat_cor(method = "pearson", label.x = xpoz, label.y = ypoz) +
    geom_hline(yintercept = 0, colour = "grey50") +
    geom_vline(xintercept = 0, colour = "grey50") +
    ggtitle(paste0("Change vs level obesity in ", ifelse(my_sex == "female", "women", "men"), ", dropping those with <5% obesity in ", plot.start.year)) +
    xlab(paste0("Level of ", ifelse(my_variable == "prev_bmi_l185", "underweight", "obesity"), " in ", plot.start.year," (%)")) +
    ylab(paste0("Change ", ifelse(my_variable == "prev_bmi_l185", "underweight", "obesity"), " ", plot.start.year, "-", plot.end.year," (percentage points)"))  
  ...
  ps[[paste(my_sex, "timechange obesity vs timechange underweight")]] <- ggscatter(data_timechange_v_timechange %>% filter(sex == my_sex), 
                                                                                   y = "prev_bmi_30", x = "prev_bmi_l185",
                                                                                   add = "reg.line",
                                                                                   add.params = list(color = "blue", fill = "lightgray"),
                                                                                   conf.int = TRUE) +
    ylab(paste0("Change obesity ", plot.start.year, "-", plot.end.year," (percentage points)")) +
    xlab(paste0("Change underweight ", plot.start.year, "-", plot.end.year," (percentage points)")) +
    stat_cor(method = "pearson", label.y = 25, label.x = -20) +
    geom_hline(yintercept = 0, colour = "grey50") +
    geom_vline(xintercept = 0, colour = "grey50") +
