...
if(age_type == "adult"){
  ...
  variables         <- c("prev_double_burden", "prev_bmi_30_proportion_double_burden")
  
} else{
  variables         <- c("prev_bmi_neg2sd", "prev_bmi_2sd")
  variables         <- c("prev_double_burden", "prev_bmi_2sd_proportion_double_burden")
}
...
for (vars in list(c(variables[1], variables[2]))){
  ...
  for(my_sex in sexes){
    ...
    saveWidget(plotly.ps[[paste("Scatter", my_sex, xvar, yvar)]], paste0(outdir_folder, "Change ", get_var_longname(yvar)," vs change ", get_var_longname(xvar), " ", my_sex, ".html"), selfcontained = F, libdir = "lib")
    }
}
