...
for (vars in list(c("prev_bmi_2sd_proportion_double_burden", "prev_double_burden"),
  ...
  for(my_sex in sexes){
    ...
    plotly.ps[[paste("Scatter", my_sex, xvar, yvar, "dynamic")]] <- interactive_scatter(subset = data_level, xvar = xvar, yvar = yvar, size_var = "prev_double_burden", sex = my_sex,
                                                                                        age_group = my_age_group, plot.year = NULL, dynamic =T)
