...
for (my_country in unique(countrylist$Country)){
  p <- ggplot(data_all %>% filter(Country == my_country & year >= start.year.plot & year <= end.year.plot), aes(x = year, y = prevalence, fill = variable)) +
...
cairo_pdf(paste0(outdir, "Trends in ", ifelse(type=="all", "all prevalence categories", type), " by country ", ifelse(sex == "female", "girls", "boys"),".pdf"), height = 15, width = 15, onefile=T)
...
for (my_region in region_order){
  ...
  plot.title <- paste0("Sex: ", plot.title <- switch(sex, "female" = "Girls", "male" = "Boys"), "\nSuperregion: ", my_sregion, "\nRegion: ", my_region)
